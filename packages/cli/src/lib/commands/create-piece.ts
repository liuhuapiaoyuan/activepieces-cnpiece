...
const BASEPATH = path.join(cwd() , "../../packages")
...
const checkIfPieceExists = async (pieceName: string) => {
  ...
  if (!path) {
    console.log(chalk.red(`🚨 Piece not exists at ${path} , please create this dir`));
    process.exit(1);
  }
};
...
const nxGenerateNodeLibrary = async (
  ...
  const nxGenerateCommand = [
    ...
    `--directory=${BASEPATH}${pieceType}/${pieceName}`,
...
const removeUnusedFiles = async (pieceName: string, pieceType: string) => {
  const path = `${BASEPATH}/${pieceName}/src/lib/`;
...
const generateIndexTsFile = async (pieceName: string, pieceType: string) => {
  ...
  await writeFile(
    `${BASEPATH}/${pieceName}/src/index.ts`,
    indexTemplate
  );
};
...
export const createPiece = async (
  ...
  console.log(
    chalk.yellow(
      `The piece has been generated at: ${BASEPATH}${pieceType}/${pieceName}`
    )
  );
};
